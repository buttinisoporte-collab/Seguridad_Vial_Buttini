...
import type { Siniestro, Position, Route, Risk, RiskType } from '../types';
import { ShieldAlert, Trash2, Folder, MapPin, Edit, Image as ImageIcon } from 'lucide-react';
...
export const SiniestrosAdmin: React.FC<SiniestrosAdminProps> = ({ 
    siniestros, routes, riskTypes, risks = [], handleDeleteSiniestro, handleDeleteRisk, selectedSiniestroId, setSelectedSiniestroId, onFocusPosition, onUpdateSiniestro, onEditSiniestroFull, onAddManualSiniestro
}) => {
    ...
    const sortedSiniestros = useMemo(() => {
        const unifiedList: any[] = [...siniestros.map(s => ({...s, isLegacy: false}))];
        ...
        return unifiedList.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
    }, [siniestros, risks, riskTypes]);
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex justify-between items-start mb-2">
                <h2 className="text-xl font-bold text-red-400 flex items-center gap-2"><ShieldAlert /> Reportes de Siniestros</h2>
                {onAddManualSiniestro && <button onClick={onAddManualSiniestro} className="bg-red-600 hover:bg-red-500 text-white text-xs font-bold px-3 py-2 rounded shadow-md whitespace-nowrap">+ Añadir Siniestro</button>}
            </div>
            <p className="text-xs text-gray-400 mb-4">Haga clic en el mapa de la derecha para ubicar un nuevo siniestro, o presione "+ Añadir Siniestro".</p>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {sortedSiniestros.length === 0 ? (
                    ...
                    sortedSiniestros.map(sin => {
                        if (sin.isLegacy) {
                            return (
                                <div key={sin.id} className={`bg-gray-800 border transition-colors rounded-lg shadow-md overflow-hidden ${selectedSiniestroId === sin.id ? 'border-orange-500' : 'border-gray-700'}`}>
                                    <div className="p-4 cursor-pointer hover:bg-gray-750 flex justify-between items-center" onClick={() => { setSelectedSiniestroId(selectedSiniestroId === sin.id ? null : sin.id); if (sin.position?.lat && sin.position?.lng) onFocusPosition({ lat: sin.position.lat, lng: sin.position.lng }); }}>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="bg-orange-900 text-orange-300 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide">Siniestro Antiguo</span>
                                                <span className="text-xs text-gray-400">{sin.timestamp ? new Date(sin.timestamp).toLocaleString('es-AR') : 'Sin fecha'}</span>
                                            </div>
                                            <h3 className="font-bold text-white text-sm">{sin.riskTypeObj?.name || 'Siniestro'}</h3>
                                            {sin.driverReportDetails && <p className="text-[10px] text-gray-400">Línea: {sin.driverReportDetails.linea} | Conductor: {sin.driverReportDetails.conductorName}</p>}
                                        </div>
                                        {handleDeleteRisk && <button onClick={(e) => { e.stopPropagation(); handleDeleteRisk(sin.id); }} className="text-gray-500 hover:text-red-500 p-2"><Trash2 size={16} /></button>}
                                    </div>
                                    {selectedSiniestroId === sin.id && (
                                        <div className="p-4 border-t border-gray-700 bg-gray-900 text-sm space-y-4">
                                            {sin.description && <div className="bg-gray-800 p-2 rounded"><p className="text-gray-300 italic">"{sin.description}"</p></div>}
                                            {(Array.isArray(sin.images) && sin.images.length > 0) && (<div><h4 className="text-orange-400 font-bold mb-2 border-b border-gray-700 pb-1">Fotografías ({sin.images.length})</h4><div className="grid grid-cols-3 gap-2">{sin.images.map((img: string, idx: number) => (<a key={idx} href={img} target="_blank" rel="noreferrer"><img src={img} alt="Siniestro Legacy" className="w-full h-16 object-cover rounded border border-gray-600 hover:border-orange-500" /></a>))}</div></div>)}
                                ...
                                <div className="p-4 border-t border-gray-700 bg-gray-900 text-sm space-y-4">
                                    <div className="flex justify-between items-start border-b border-gray-700 pb-2">
                                        ...
                                        <div className="flex gap-2">
                                            {onEditSiniestroFull ? (
                                                <button onClick={() => onEditSiniestroFull(sin)} className="flex items-center gap-1 text-[10px] bg-sky-900/50 hover:bg-sky-900 px-2 py-1 rounded text-sky-400"><Edit size={12}/> Editar Completo</button>
                                            ) : (
                                                <button onClick={() => startEdit(sin.id, sin.timestamp, sin.associatedRouteId)} className="flex items-center gap-1 text-[10px] bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-sky-400"><Edit size={12}/> Editar Rápido</button>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {(Array.isArray(sin.images) && sin.images.length > 0) && (<div><h4 className="text-sky-400 font-bold mb-2 border-b border-gray-700 pb-1">Fotografías ({sin.images.length})</h4><div className="grid grid-cols-3 gap-2">{sin.images.map((img: string, idx: number) => (<a key={idx} href={img} target="_blank" rel="noreferrer"><img src={img} alt="Siniestro" className="w-full h-16 object-cover rounded border border-gray-600 hover:border-sky-500" /></a>))}</div></div>)}
                                    {sin.driveUrl && (<div className="pt-2"><a href={sin.driveUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-xs"><Folder size={16}/> Carpeta Externa Drive</a></div>)}
                                </div>
                                )
                            )}
