...
interface RouteManagerProps {
    ...
    setActiveRouteId: (v: string | null) => void;
}
...
export const RouteManager: React.FC<RouteManagerProps> = ({ 
    ...
    const handleFilterChange = (setter: (v: string) => void, value: string) => {
        setter(value);
        setActiveRouteId(null); // Quitar selección única si se cambia el filtro
    };
        ...
        <div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
                ...
                <input
                    ...
                    value={filterLine}
                    onChange={(e) => handleFilterChange(setFilterLine, e.target.value)}
                    className="bg-gray-700 text-white text-xs p-2 rounded border border-gray-600 focus:ring-sky-500 focus:border-sky-500"
                />
                <input
                    type="text"
            ...
            <div className="space-y-3">
                {filteredRoutes.length > 0 ? filteredRoutes.map(route => (
                    <div 
                        ...
                        className={`p-3 rounded-lg flex justify-between items-center cursor-pointer transition-colors border ${activeRouteId === route.id ? 'bg-sky-900 border-sky-500' : 'bg-gray-700 border-transparent hover:bg-gray-600'}`}
                    >
                        ...
                        <button onClick={(e) => { e.stopPropagation(); deleteRoute(route.id); }} className="text-red-400 hover:text-red-600 p-2 ml-2">
                            <Trash2 size={18} />
                        </button>
                    </div>
                )) : (
