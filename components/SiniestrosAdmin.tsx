                                    ...
                                    {selectedSiniestroId === sin.id && (
                                        ...
                                        <div className="p-4 border-t border-gray-700 bg-gray-900 text-sm space-y-4">
                                            <div className="flex justify-between items-start border-b border-gray-700 pb-2">
                                                ...
                                                <button onClick={() => startEdit(sin.id, sin.timestamp, sin.associatedRouteId)} className="flex items-center gap-1 text-[10px] bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-sky-400"><Edit size={12}/> Editar</button>
                                            </div>
                                    ...
                                    <div className="p-4 cursor-pointer hover:bg-gray-750 flex justify-between items-center" onClick={() => { setSelectedSiniestroId(selectedSiniestroId === risk.id ? null : risk.id); if (risk.position) onFocusPosition(risk.position); }}>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-xs text-gray-400">{risk.timestamp ? new Date(risk.timestamp).toLocaleString('es-AR') : <span className="text-orange-400 font-bold">Sin fecha asignada</span>}</span>
                                            </div>
                                    ...
                                    {selectedSiniestroId === risk.id && (
                                        ...
                                        <div className="p-4 border-t border-gray-700 bg-gray-900 text-sm space-y-4">
                                            <div className="flex justify-between items-center border-b border-gray-700 pb-2">
                                                <p className="text-[11px] text-gray-400"><b>Ruta Afectada:</b> {risk.associatedRouteIds?.[0] ? routes.find(r=>r.id===risk.associatedRouteIds[0])?.name : 'No asignada'}</p>
                                                <button onClick={() => startEdit(risk.id, risk.timestamp, risk.associatedRouteIds?.[0])} className="flex items-center gap-1 text-[10px] bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-sky-400"><Edit size={12}/> Editar Fecha/Ruta</button>
                                            </div>
                                            ...
                                            {(Array.isArray(risk.images) && risk.images.length > 0) && (<div><h4 className="text-orange-400 font-bold mb-2 border-b border-gray-700 pb-1">Fotografías ({risk.images.length})</h4><div className="grid grid-cols-3 gap-2">{risk.images.map((img, idx) => (<a key={idx} href={img} target="_blank" rel="noreferrer"><img src={img} alt="Siniestro" className="w-full h-16 object-cover rounded border border-gray-600 hover:border-sky-500" /></a>))}</div></div>)}
                                            {risk.driveUrl && (<div className="pt-2"><a href={risk.driveUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-xs"><Folder size={16}/> Carpeta Externa Drive</a></div>)}
                                        </div>
                                    ))}
                                </div>
                            );
